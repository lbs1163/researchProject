...
function features_pos = get_strong_positive_features(VOCopts, cls, feature_params)
...
for i = 1 : num_images
    ...
    if gt==1
        ...
        image_boxes = rec.objects(clsinds).bbox;
        
        for j=1:size(image_boxes, 1)
            template = imresize(image(image_boxes(j,2):image_boxes(j,4), image_boxes(j,1):image_boxes(j,3), :), [72 72]);
            template_hog=vl_hog(template, feature_params.hog_cell_size);
            features_pos=[features_pos;reshape(template_hog, 1, [])];
        end
    end
end

save('features_pos_strong.mat', 'features_pos', '-v7.3');
