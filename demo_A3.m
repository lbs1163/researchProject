...
data_path = 'VOCdevkit/VOC2007/JPEGImages'; %change if you want to work with a network copy
test_scn_path = data_path;
resultPath = 'results/VOC2007';
...
% 'hog_dimension', 31, 'threshold', -1, 'svm_lambda', 0.0001, ...
% 'num_negative_examples', 10000, 'num_hard_negative_iteration', 0, ...
...
try
    ...
    features_pos = get_positive_features(VOCopts, cls, feature_params ); % N*D
end

try
    load('features_rand_neg.mat');
catch
    num_negative_examples = feature_params.num_negative_examples; %Higher will work strictly better, but you should start with 10000 for debugging
    features_neg = get_random_negative_features(VOCopts, cls, feature_params, num_negative_examples); % N(=10000) * D
end
...
try
    ...
    for t=1:T
        ...
        %refinement = cell(10*size(features_pos, 2));
        %for n=1:size(features_pos, 2)
        %    sorted=sort(features_pos{n}, 'descend');
        %    refinement{n}=sorted(1:10, :);
        %end
        %refinement_mat=cell2mat(refinement');
        %m=[repelem(1, size(refinement_mat, 1)), repelem(-1, size(features_neg, 1))]';
        %fprintf('iteration %d svm train with refined window\n', t);
        %[w b] = vl_svmtrain([refinement_mat' features_neg'], [repelem(1, size(refinement_mat, 1)), repelem(-1, size(features_neg, 1))]', feature_params.svm_lambda);
        ...
        confidences = [features_pos_mat; features_neg]*w + b;
        label_vector = [ones(size(features_pos_mat,1),1); -1*ones(size(features_neg,1),1)];
        ...
        % your writeup!
        n_hog_cells = sqrt(length(w) / feature_params.hog_dimension); %specific to default HoG parameters
        imhog = vl_hog('render', single(reshape(w, [n_hog_cells n_hog_cells feature_params.hog_dimension])), 'verbose') ;
